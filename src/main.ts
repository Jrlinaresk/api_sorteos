...
async function bootstrap() {
  ...
  if (config.get<string>('TRUST_PROXY') === 'true') express.set('trust proxy', 1);
  ...
  if (config.get<string>('SWAGGER_ENABLED') !== 'false') {
    const swaggerConfig = new DocumentBuilder()
      ...
      .setDescription('Campañas, checkout Pix, cuotas, resultados y administración')
      .setVersion('1.0.0')
      .addBearerAuth()
      .addApiKey({ type: 'apiKey', in: 'header', name: 'X-Order-Token' }, 'order-token')
      .build();
