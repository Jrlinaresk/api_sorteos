...
export function MediaPage() {
  ...
  const upload = useMutation({
    mutationFn: (form: FormData) =>
      api.post<AdminMediaAsset>('/admin/media', form),
    onSuccess: async () => {
      await Promise.all([
  ...
  const copyUrl = async (asset: AdminMediaAsset) => {
    const relative =
      asset.publicUrl || asset.url || `/api/v1/media/${asset.id}`;
  ...
  return (
    <main className="page">
      ...
      {usage.data ? (
        ...
        >
          <UsageCard
            title="Almacenamiento global"
            counter={usage.data.global}
          />
          <UsageCard
            title="Tu almacenamiento"
            counter={usage.data.currentUser}
          />
        </section>
      ) : null}
      {usage.isError ? (
        <ErrorPanel error={usage.error} onRetry={() => usage.refetch()} />
      ) : null}
      ...
      >
        <div>
          ...
          <p>
            JPEG, PNG, WebP, MP4 o WebM. El servidor valida firma, tamaño y
            cuota.
          </p>
        </div>
        <form
          ...
          onSubmit={(event) => {
            ...
            submitForm(event, (form) => {
              ...
              upload.mutate(form, { onSuccess: () => formElement.reset() });
            });
          }}
        >
      ...
      <section className="panel" aria-label="Archivos almacenados">
        <div className="filters">
          ...
          <label>
            ...
            <select
              value={referenced}
              onChange={(event) => setFilter(setReferenced, event.target.value)}
            >
              <option value="">Todos</option>
        ...
        {assets.isError ? (
          <ErrorPanel error={assets.error} onRetry={() => assets.refetch()} />
        ) : null}
        {assets.isSuccess && !assets.data.data.length ? (
          <EmptyPanel
            title="No hay archivos con estos filtros"
            description="Cambia los filtros o sube un nuevo archivo."
          />
        ) : null}
          ...
          <div className="media-grid">
            {assets.data.data.map((asset) => {
              const url =
                asset.publicUrl || asset.url || `/api/v1/media/${asset.id}`;
              const references =
                asset.referenceCount ?? asset.references?.length ?? 0;
              return (
                    ...
                    <div className="card-actions">
                      ...
                      {can('admin') && asset.status === 'active' ? (
                        <button
                          ...
                          title={
                            references
                              ? 'El archivo tiene referencias activas'
                              : undefined
                          }
                      ...
                      {can('admin') && asset.status === 'deleted' ? (
                        <button
                          className="button button--danger-ghost"
                          type="button"
                          onClick={() => setAction({ kind: 'purge', asset })}
                        >
  ...
  return (
    <article className="metric-card storage-card">
      ...
      <div>
        <span>{title}</span>
        <strong>
          {formatBytes(counter.bytes)} / {formatBytes(counter.maxBytes)}
        </strong>
        <div
          className="progress"
          role="progressbar"
          aria-label={`${title}: ${used.toFixed(0)}% usado`}
          aria-valuenow={used}
          aria-valuemin={0}
          aria-valuemax={100}
        >
        ...
        <small>
          {counter.objects} archivos · {formatBytes(counter.remainingBytes)}{' '}
          disponibles
        </small>
      </div>
    </article>
